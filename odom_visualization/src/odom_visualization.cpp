...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (tf45) {
    ...
    string laser_s =
        _drone_id == -1 ? string("laser") : string("laser") + std::to_string(_drone_id);
    string vision_s =
        _drone_id == -1 ? string("vision") : string("vision") + std::to_string(_drone_id);
    string height_s =
        _drone_id == -1 ? string("height") : string("height") + std::to_string(_drone_id);

    broadcaster->sendTransform(
        tf::StampedTransform(transform, msg->header.stamp, string("world"), base_s));
    broadcaster->sendTransform(
        tf::StampedTransform(transform45, msg->header.stamp, base_s, laser_s));
    broadcaster->sendTransform(
        tf::StampedTransform(transform45, msg->header.stamp, base_s, vision_s));
    broadcaster->sendTransform(
        tf::StampedTransform(transform90, msg->header.stamp, base_s, height_s));
  }
}
